...
\begin{document}
%
\title{Linear Attention Architectures for Geopotential Height Forecasting: A Comparative Study}
%
...
\begin{enumerate}
    ...
    \item Analysis of the accuracy-efficiency trade-offs across architectures, measuring forecasting skill (RMSE, anomaly correlation), computational throughput, and memory consumption.
    \item An open-source training framework, \method{}, built on torchtitan with specialized support for linear attention models, enabling reproducible research in weather forecasting.
\end{enumerate}
...
State space models (SSMs) offer an alternative approach to efficient sequence modeling~\cite{gu2022efficiently}. The Structured State Space (S4) model~\cite{gu2022efficiently} parameterizes a continuous-time linear system:
\begin{equation}
...
\begin{figure}[t]
    ...
    \fbox{\parbox{0.85\textwidth}{\centering [Architecture diagram placeholder: Input frames $\rightarrow$ Projection $\rightarrow$ Attention Backbone $\rightarrow$ Forecast Head $\rightarrow$ Output frames]}}
